        ...
        <c:choose>
          <c:when test="${item.order_id != pre_order_id }">   
            <tr>       
				...
				<td>
				   <strong>
				      <c:forEach var="item3" items="${myOrderHistList}" varStatus="j">
				          <c:if  test="${item.order_id ==item3.order_id}" >
				             ${item3.goods_sales_price*item3.order_goods_qty }원/${item3.order_goods_qty }<br>
				         </c:if>   
					 </c:forEach>
				   </strong>
				</td>
